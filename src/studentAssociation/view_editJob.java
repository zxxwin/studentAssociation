...
public class view_editJob extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					view_editJob frame = new view_editJob(stuInAssFrame,stuId);
	 ...
	 * Create the frame.
	 */
	public view_editJob(view_stuInAss stuInAssFrame,String stuId) {
		setTitle("\u52A0\u5165\u793E\u56E2");
...
//		System.out.println(recordNum);
		...
		int row = stuInAssFrame.table.getSelectedRow();
		Object selectedValue = stuInAssFrame.table.getValueAt(row, 1);
		
		assName.setModel(new DefaultComboBoxModel(assNames));
//		assName.setSelectedIndex(0);
		assName.setSelectedItem(selectedValue);
		assName.setEnabled(false);
...
//				System.out.println(assId+"??????");
				controller.setStudentJob(stuId + " " + assId + " "  + jobValue);
				controller.saveData();
				stuInAssFrame.updataTable();
				JOptionPane.showMessageDialog(rootPane, "ÐÞ¸Ä³É¹¦£¡");
			}
		});
